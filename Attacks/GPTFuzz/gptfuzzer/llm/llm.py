...
class LocalLLM(LLM):
    ...
    @torch.inference_mode()
    def generate(self, prompt, temperature=0.01, max_tokens=256, repetition_penalty=1.0):
        model_name = 'vicuna' if 'vicuna' in self.model_path else 'llama-2'
        conv_temp = get_conversation_template(model_name)
    ...
    @torch.inference_mode()
    def generate_batch(self, prompts, temperature=0.01, max_tokens=256, repetition_penalty=1.0, batch_size=16):
        prompt_inputs = []
        model_name = 'vicuna' if 'vicuna' in self.model_path else 'llama-2'
...
class OpenAILLM(LLM):
    def __init__(self,
        ...
        self.model_path = model_path
        # print("your model path is",model_path)
    ...
    def generate(self, prompt, temperature=0, max_tokens=512, n=1, max_trials=10, failure_sleep_time=5):
        for _ in range(max_trials):
            try:
                ...
                return [results.choices[0].message.content for i in range(n)]
            except Exception as e:
                logging.warning(
