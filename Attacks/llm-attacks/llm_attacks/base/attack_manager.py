...
class MultiPromptAttack(object):
    ...
    def get_filtered_cands(self, worker_index, control_cand, filter_cand=True, curr_control=None):
        ...
        if not cands:
            print(f"No valid control candidates found. Worker index: {worker_index}")
            print(f"Control candidates count: {control_cand.shape[0]}, Filtered count: {count}")
            print("Content of cands: (empty)")
            print(f"Content of control_cand: {control_cand[0]}")
            # exclamation_string = " ".join("!" for _ in range(20))
            cands.append("! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! ! !")   
        
        if filter_cand:
            if cands:
                cands = cands + [cands[-1]] * (len(control_cand) - len(cands))
            # Optionally print a warning about the number of candidates filtered out
    ...
    def run(self, 
        ...
        for i in range(n_steps):
            print(f"running steps {i} / {n_steps}")
            if stop_on_success:
                ...
                if all(all(tests for tests in model_test) for model_test in model_tests_jb):
                    ## individual testcase
                    prompt_count += 1
                    print("Success!" ,prompt_count)
                    success_data = {
                        'prompt': self.prompts[0].control_str,
                        'response': "Verify Later",
                        'template_name': "gradient",
                        'type': 'GCG',
                        'question': self.goals[0],
                        'status':True,
                    }
                    success_prompts.append(success_data)
                    break
            success_data = {
                    'prompt': self.prompts[0].control_str,
                    'response': "Verify Later",
...
class IndividualPromptAttack(object):
    ...
    def run(self, 
        ...
        for i in range(len(self.goals)):
            ...
            if self.success_count == prompt_count:
                break
        if prompt_count > 0:
            with open (f'../../../Results/GCG_{self.model_name}', 'w') as f:
                    json.dump(success_prompts, f, indent=4)
...
def get_workers(params, eval=False):
    ...
    for i in range(len(params.tokenizer_paths)):
        print("current path")
        print(os.getcwd())
        print("loaded model from the path")
...
def get_goals_and_targets(params):
    ...
    if params.train_data:
        params.train_data = "../../../Data/data.csv"
